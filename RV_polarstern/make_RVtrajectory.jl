...
# ╔═╡ e26dddf4-41ff-4a54-b0e8-2936f190f36c
function get_∇WVT(Pa::Matrix, WS::Matrix, Qv::Matrix, H::Vector)
	...
	∇Φ_qv *= 1f3   # [g m⁻² s-¹]
	#tmp = -vcat([Qv[i,:].*WS[i,:].*Δz[i,:]  for i=1:200]'...) #
	#return (tmp .+ vcat([sum(∇Φ_qv[i:200,:], dims=1) for i=1:200]...))
    return ∇Φ_qv
end

# ╔═╡ b842e1dd-efd4-4226-9dd0-7ac2f716ba73
WVT = get_∇WVT(rs[:Pa], rs[:WSPD], rs[:qv], rs[:height]);

# ╔═╡ d31f93c6-e5c2-4303-8c1c-14979d1a2dae
size(WVT)

# ╔═╡ 204b4ea8-dc50-4de2-82d0-bb3767168153
begin
	
	IVT, _ = ATM.getIVT(rs)
	Plots.plot(WVT[:, [1, 100, 1000, 1440]], rs[:height][1:end-1], ylim=(0, 8), xlim=(-0.1, 0.5))
end

# ╔═╡ f350b373-dff8-41e9-9689-0b44af3f020a
Plots.heatmap(rs[:time], rs[:height][1:end-1], WVT, ylim=(0, 8), color=:fall, size=(900,400)); Plots.plot!(rs[:time], [PBLH H_wvt INV[:HEIGHT][1,:].+INV[:ΔHEIGHT][1,:]], lc=[:black :red :lightblue], label=["PBLH" "max_WVT" "Inv_H"])

# ╔═╡ 6aa00dd6-e889-4240-9c1c-e97dad55f2e0
INV[:ΔT];

# ╔═╡ 5e9510d9-6452-46b0-8556-2310b6db8166
Plots.heatmap(rs[:time], rs[:height][1:235], IVT, ylim=(0,8), size=(900,400))
...
begin
	imov = 1000 #idx_nav[1]
		...
		GMT.scatter(lon, lat, color=sic_bar, zcolor=SIC, markersize=0.15, marker=:square,
			region = (LonLim..., LatLim...), frame=:g, figsize=(1.5, 6),
		...
		GMT.coast!(
			...
			res=:high, area=950, land=:lightbrown, figsize=(1.5, 6), #transparency=9, #figscale="1:1900000",
		...
		GMT.plot!(Pstern.λ, Pstern.ϕ, symbol=(symb=:asterisk, size=.3), mc=:red, markersize=1.8, show=0) #, label="RV")
		lon_lines_mov, lat_lines_mov = SEAICE.create_pair_lines(Pstern, 1e3wind_range[imov], wind_dir[imov])
		GMT.plot!(lon_lines_mov, lat_lines_mov,
			region = (LonLim..., LatLim...),
			prog =(name=:stereographic, center = [Pstern.λ, 90]),
			alpha=60, lc=:gray3, show=1, fmt=:png) #, label="Wind dir.", savefig="./plots/legend_$(imov)_MOSAiC_SIC_wdir_$(yy).$(mm).$(dd).png", legend=(justify=:TL)) #,
