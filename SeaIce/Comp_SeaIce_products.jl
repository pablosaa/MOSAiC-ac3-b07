...
begin
    ...
    const BASE_PATH = joinpath(homedir(), "LIM/data") #joinpath("/media/psgarfias/LaCie SSD/LIM/data/", CAMPAIGN); #
...
begin
    ...
    # idx_box is the indexes of SIC coordinates inside the LonLim, LatLim Box:
...
begin
    ...
    sic_bar = GMT.makecpt(cmap=:vik, truncate=(-1, 0), range=(80, 100, 2), continues=true, nobg=true)	
    # creating title:
    ...
    GMT.scatter(lon[iprod], lat[iprod], color=sic_bar, zcolor=SIC[iprod], markersize=pixel_size[SENSOR[iprod]],
		marker=:square, region = (LonLim..., LatLim...), frame=:g, figsize=(1.5, 7),
	...
	GMT.colorbar!(pos=(outside=:CR, length=(5,0.15), offset=(.3 ,-.5)), color=sic_bar, frame=(ylabel="%",), show=0)
    GMT.plot!(Pstern.λ, Pstern.ϕ, symbol=(symb=:asterisk, size=.3), mc=:red, show=0)
	GMT.plot!(lon_circ, lat_circ, lc=:gray1, lw=0.5, ls=:dash, alpha=50, show=0, label="50km ø")
    GMT.plot!(RV[:lon], RV[:lat], linestyle=:line, lw=0.4, lc=:lightred, show=1, legend=(label="Drift", pos=:TL), fmt=:png)
    
		#legend=(label="Polarstern", pos=:TL, offset=(0, -2)), fmt=:png)
...
# ╔═╡ 802cbb69-646c-4797-aa2e-005ae413b45e
begin
	P0=Dict{Symbol, Any}(:lon=>Pstern.λ, :lat=>Pstern.ϕ, :label=>"")
	Circ=Dict(:lon=>lon_circ, :lat=>lat_circ, :label=>"50km ø")
	RVPolarstern=Dict{Symbol, Any}(:lon=>RV[:lon], :lat=>RV[:lat])
	# defining color scheme:
    sic_bar2 = GMT.makecpt(cmap=:vik, truncate=(-1, 0), range=(75, 100, 5), continues=true, nobg=true)
	draw_MOSAiC_sic(lon[iprod], lat[iprod], SIC[iprod], P0, pixel_size=pixel_size[SENSOR[iprod]], RV=RVPolarstern, Circ=Circ, title_str=wind_title_str, CBar=sic_bar2)
end

# ╔═╡ 094ebffe-1eec-49d6-886b-ae30db37604b
begin
	GMT.histogram(SIC[1], normal=0, kind=4, center=true, bin=0.5, figscale=(0.8,0.9), fmt=:png, show=0)
	GMT.histogram!(SIC[2], normal=0, kind=4, center=true, bin=0.5, fill=:lightgreen, lc=:black, fmt=:png, show=1)
end

# ╔═╡ 578c2586-44d0-4068-853d-ce32c29c79f2
begin
	SIC_ASI = Vector{Float32}(undef, 0)
	idx_within = Vector{Vector{Any}}(undef, 0)
	ωi = Vector{Vector{Float32}}(undef, 0)
	μ_SIC = Vector{Vector{Float32}}(undef, 0)
	
	foreach(sic_coor[2][idx_box[2]]) do P0
		let Pin= sic_coor[1][idx_box[1]] |> x-> reshape(x, length(x), 1)
			θ, ρ_km = SEAICE.LonLat_To_CenteredPolar(P0, Pin)
			Ji = findall(≤(10), ρ_km)
			push!(idx_within, Ji)
			push!(ωi, 1.0./ρ_km[Ji])
			push!(μ_SIC, SIC[1][Ji])
			push!(SIC_ASI, StatsBase.mean(SIC[1][Ji], StatsBase.weights(ωi[end])))
		end
		#push!(SIC_ASI, [StatsBase.mean(SIC[1][Ji], StatsBase.weights(Wi)), std(SIC[1][idx_within], weights(Wi)), length(idx_within)])
	end
	#SIC_ASI = hcat(SIC_ASI...)'
end

# ╔═╡ eb94b16b-2b0a-4031-b289-88b953e7cd95
@bind idxplt Slider(1:100, show_value=true)
...
# ╔═╡ 19f1d24a-937a-4c0b-9556-8d8bf57ff22b
Plots.plot(mycdf(μ_SIC[idxplt]), label="asi", xlim=(90,100)); Plots.vline!([SIC_ASI[idxplt] SIC[2][idxplt]], label=["μ_asi" "μ_osi"], legend=:topleft)

# ╔═╡ 60e6794f-ec37-4de0-aa7b-c27384eb3221
Plots.plot(mycdf(SIC[2]), legend=:outerleft); Plots.plot!(mycdf(SIC_ASI[:,1])) #; #Plots.plot!(mycdf(new_asi))

# ╔═╡ 3f6d5e59-339f-420e-bf4c-3861fe0d10f7
osi_lim = extrema(SIC[2]) #; asi_lim = extrema(SIC_ASI)

# ╔═╡ aed7975f-0170-49cb-8347-6072d9392887
@bind factor Slider(1:0.02:1.1; default=1, show_value=true)

# ╔═╡ ed5604d6-2632-4722-b48d-e2878e556e17
Plots.plot(mycdf(SIC[2]), label="osi"); Plots.plot!((1, factor).*mycdf(SIC[1]), label="asi")

# ╔═╡ ecfb4878-327a-403a-b8e0-7ad74027609d
Plots.scatter(SIC_ASI, SIC_ASI.-SIC[2], m=:c, value_z=SIC[2])

# ╔═╡ 8ee90d10-ee3d-4dbf-a866-06305b32d47a
typeof(SIC[2]), typeof(μ_SIC)

# ╔═╡ 1000d018-8615-4a06-93b5-1dca542ac800
##begin
## # For trajectories of specific humidity Q at 100 [m]
##	const backtrajectory = "/home/psgarfias/LIM/data/synoptic/Meteo_Q00100m.nc";
##	
##	Qtraj =	NCDataset(backtrajectory) do nc
##	
##	time = nc["time"]
##	time_in = let thisday=DateTime(yy,mm,dd)
##		findall(thisday .≤ time .< (thisday + Day(1) + Minute(1)))
##	end
##	lon = nc["lon"]
##	lat = nc["lat"]
##	
##	lon_in = findall(LonLim[1] .≤ lon .≤ LonLim[2])
##	lat_in = findall(LatLim[1] .≤ lat .≤ LatLim[2])
##	
##		Dict(
##			:time => time[time_in],
##			:lat => nc["lat"][lat_in],
##			:lon => nc["lon"][lon_in],
##			:age => nc["age"],
##			:qv => nc["Q"][lon_in, lat_in, 1:2, time_in]
##		)
##	end;
##		end;
...
git-tree-sha1 = "0f24de8ff08c49de06bb88936f895b71d0d7a221"
repo-rev = "main"
...
git-tree-sha1 = "ae870b901b89a528c59c8ceb9c4fc98ba2c4574f"
